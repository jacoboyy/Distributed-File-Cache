 ...
 * Implementation of a remote server that utilizes java RMI to serve read/write/unlink RPCs from a
 * caching proxy. The server can handle multiple proxies concurrently.
 */
...
public class Server extends UnicastRemoteObject implements RMIInterface {
  ...
  /**
   * Read a file on server and send its file content back only if the proxy version doesn't exist or
   * is outdated. The size to content read is capped by the chunk size.
   * @param path              file name
   * @param o                 open mode
   * @param versionOnProxy    version of file on proxy, -1 if file doesn't exist
   * @param offset            offset to start reading the file
   * @return a ServerFile object, with the file content if the read is sucessful or the negative
   *     errno if read fails.
   */
  public synchronized ServerFile readServerFile(
      String path, FileHandling.OpenOption o, int versionOnProxy, int offset) {
      ...
      try {
        if (file.exists()) {
          ...
          return new ServerFile(
              new byte[0], pathToVersion.containsKey(path) ? pathToVersion.get(path) : 0, 0);
        }
      } catch (Exception e) {
   ...
   * @param path  server file path
   * @return 0 if operation is successful, or a nagative number indicating the error number
   *     otherwise
   */
