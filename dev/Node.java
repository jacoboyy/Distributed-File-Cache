...
class Node {
   ...
   * A node is evictable if and only if there is no current operation on the file (reference count =
   * 0).
   */
  ...
  /**
   * Update the name of the file, used when close is called on a file that has been modified by a
   * client and changes are propagated back to the server. Also update its readability as the file
   * becomes visible to all clients.
