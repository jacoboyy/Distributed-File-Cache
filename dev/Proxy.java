...
class Proxy {
  ...
  private static class FileHandler implements FileHandling {
     ...
     */
    public synchronized ServerFile readServerFile(
        String path, OpenOption o, int version, int offset) {
     ...
     * @return a positive number representing the new file version if write the successful,
    ...
    /**
     * Open a file on server specified by the file path and open option. Only fetch from server if
     * the file doesn't exist on the proxy or the proxy copy is outdated, otherwise directly read
     * from proxy copy.
     * @param path	relative file path
     * @param o	    open mode
     * @return a positive number for the file descriptor if the operation is successful, otherwise a
     *     negative errno
     */
    public synchronized int open(String path, OpenOption o) {
      ...
      synchronized (cache) {
        ...
        ServerFile response =
            readServerFile(normalizedPath, o, (node != null) ? node.version : -1, 0);
          ...
          if (node != null && response.version == node.version) {
            ...
            try {
              ...
              while (offset < fileSize) {
                response =
                    readServerFile(normalizedPath, o, (node != null) ? node.version : -1, offset);
                file.write(response.content);
    ...
    /**
     * Close a open file descriptor. If the proxy copy is written to, propagate changes back to
     * server and update the server copy. The proxy copy will also be made visible by all clients.
     * All stale copies of the file will be removed.
    ...
    /**
     * Write to a file indicated by the file desciptor. A new copy of the file will be created and
     * added to the cache on the first write to the file descriptor, which is only visible to the
     * client that performs the write.
    ...
    public synchronized long write(int fd, byte[] buf) {
      ...
      if (!fdToNode.containsKey(fd) || readOnlyFds.contains(fd))
        return Errors.EBADF;
      long fileSize;
     ...
     * @return the number of bytes actually read on success, or a negative value indicating the
     *     error
     */
    public synchronized long read(int fd, byte[] buf) {
      ...
      try {
        readRes = fdToRead.containsKey(fd) ? (long) fdToRead.get(fd).read(buf)
                                           : (long) fdToWrite.get(fd).read(buf);
      } catch (Exception e) {
    ...
    public synchronized long lseek(int fd, long pos, LseekOption o) {
      ...
      if (o == LseekOption.FROM_START)
        ...
        long currPos;
        try {
          currPos = fdToRead.containsKey(fd) ? fdToRead.get(fd).getFilePointer()
                                             : fdToWrite.get(fd).getFilePointer();
        } catch (Exception e) {
          return -1;
        }
        seekPos = currPos + pos;
    ...
    /**
     * Delete a file on the server. Only the server copy will be directly deleted, a proxy copy is
     * either being used or will be invalided on the next open on the file.
     * @param path file path
    ...
    /**
     * End session when a client is done. Clean up all states and open files associated with the
     * client.
     */
    public void clientdone() {
      // close open files
      for (int fd : fdToNode.keySet()) this.close(fd);
      // reset state
      fdToRead.clear();
      fdToWrite.clear();
      fdToNode.clear();
  ...
  /**
   * Main entry point of the proxy program, which creates an empty cache and establishes a
